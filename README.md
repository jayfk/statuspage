...
You'll need to create an GitHub API token which is at https://github.com/settings/tokens.
Or you can navigate manually to your personal settings page on GitHub and locate 
the `Personal access tokens` tab on the left.
Then, click on `Generate new token` and give it a description. Make sure to check the `public_repo` scope.
